...
if not os.path.exists( fname ) :
  ...
  plt.xlabel( 'Angle from the Galactic Center [deg]' )
  plt.ylabel( 'Mass Density [GeV/cm^3]' )
  plt.title( 'Dark Matter Mass Density around the Galactic Center' )
  plt.xscale('log')
  plt.yscale('log')
...
if force or not os.path.exists( fname ) :
  ...
  plt.ylabel( r'J-Factor per Solid Angle $\left [\mathrm{GeV}^{2} \; \mathrm{cm}^{-5} \; \mathrm{deg}^{-2} \right ]$' )
  
  # blue rectangular analysis region
  b_xmin = amin
  b_xwid = 3 # deg
  b_ymin = 4e20 # GeV^2 cm^-5 deg^-2
  b_ywid = ( max(jfs_l_deg2) * 1.5 ) - b_ymin
  args  = [ (b_xmin, b_ymin) ]
  args += [ b_xwid ]
  args += [ b_ywid ]
  kwa  = {}
  kwa['linewidth'] = 0
  kwa['facecolor'] = 'lightblue'
  kwa['zorder'   ] = 1
  kwa['label'    ] = 'Observation Radius'
  fax.add_patch( patches.Rectangle( *args, **kwa ) )
  
  plt.xlabel( r'Angle from the Galactic Center $\left [ \mathrm{deg} \right ]$' )
  plt.title( 'Dark Matter J-Factor' )
  plt.xscale('log')
  plt.yscale('log')
  plt.xlim( 1e-2, 5 )
  plt.ylim( b_ymin, b_ymin+b_ywid )
  #legend = fax.legend( loc='lower left', shadow=True )
  fax.grid()
  plt.savefig( fname, bbox_inches='tight', dpi=300 )

print('after final jfactor plot')
