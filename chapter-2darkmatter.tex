...
\begin{figure}[h]
  ...
  \caption[Sculptor Dwarf Galaxy]{
    ...
    {\color{red}This dwarf galaxy has a $\frac{M_\odot}{L_\odot}$ ratio of $15.3\pm6.9$~\cite{sculptor_ml}. (orel highlighted this??)}
  }
  \label{fig:sculptor}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption[Fornax Dwarf Galaxy]{
    ...
    {\color{red}This dwarf galaxy has a $\frac{M_\odot}{L_\odot}$ ratio of $8.8\pm3.8$~\cite{sculptor_ml}.  (orel highlighted this??)}
  }
  \label{fig:fornax}
\end{figure}
...
\begin{table}[]
  ...
  \caption[Ratios of $\frac{M_\odot}{L_\odot}$ for Various Dwarf Galaxy Objects]{
    ...
    {\color{red}(move caption below table, and have caption start on same line as "Table 2.1." ??)}
  }
...
\begin{figure}
  ...
  \caption[Weak Lensing with an Ellipse Galaxy]{
    ...
    {\color{red}(need to reference this figure in the text??)}
  }
  \label{fig:ellipse}
\end{figure}
    
    
Galaxy lensing can be used to estimate the size of the substructure of a lensing galaxy, based on the {\color{red}4 lensed images of a quasar~\cite{weak_lensing_quasar} (is this a specific example or do you always get 4 images (and I assume it is not always a quasar)??  Need to make it clear if it is a specific example)}.
...
\begin{table}
  ...
  \caption[Ratios of \MLsol for Various Galaxy Clusters]{
    ...
    {\color{red}(need to reference this table in the text??)}
    }
...
%\begin{figure}[h]
%  \centering
%  \includegraphics[width=0.95\textwidth]{images/xkcd_dark_matter/xkcd_2035.pdf}
%  \caption[Dark Matter Candidates at Many Scales]{
%    A variety of dark matter candidates, some the subject of serious scientific searches, others more fictitious~\ref{xkcd_dark}.
%    The neutralino is a type of WIMP, the particle sought in this analysis.}
%  \label{fig:xkcd_dark}
%\end{figure}
...
%, because the particles cover a volume at a rate proportional to $t$, while the universe's expansion adds new space proportional to $a^3 \sim t^3$.
...
%waggle its eyebrows suggestively while gesturing furtively and mouthing `look over there'~\cite{xkcd_causation}.
...
%  Supersymmetry predicts the existence of a WIMP-like particle with a cross section of around \nicetilde{} \SI{3e-26}{ cm${}^2$ }~\cite{Jungman:1995df}.
%  % $3*{10}^{-26}\ {\textrm{cm}}^{2}$
