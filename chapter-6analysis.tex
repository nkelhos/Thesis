...
The analysis here adds a point source, as this produces a more conservative upper limit on the velocity-averaged cross section $\vacs$.
This occurs because, assuming the non-dm scenario but without a point source model, there are more unmodeled events.
With more unmodeled events, increasing the $\vacs$ causes the likelihood to decrease at earlier (smaller) $\vacs$, which results in smaller upper limit (see Section~\ref{upper_limit}) that eliminates more of the parameter space.
...
\begin{table}[!t]
  ...
  \caption[DM Halo TS Values]{
    ...
    {\color{red}(Shouldn't I first find the TS value without the extra point source? the point source model may be stealing events from the halo model?  then after the no-point-source TS, add point source and calc upper limit??)}
  }
  \label{tab:tsvals}
\end{table}
% 100 TeV mass limit?
