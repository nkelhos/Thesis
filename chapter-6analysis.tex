...
\begin{equation}\label{eqn:model_int}
  m_{j} = \int_{t\,\textrm{bin}} \int_{E\,\textrm{bin}} \int_{b\,\textrm{bin}} \int_{l\,\textrm{bin}} A * M(l,b,E,t)\; dl \; db \; dE \; dt \,.
\end{equation}
Here, $l$ and $b$ are sky coordinates, $E$ is energy, $t$ is time, and $A$ is the effective detector area.
...
Conceptually, the specific model being searched for should have its parameters set such that it contributes zero events, but this is mathematically the same as removing the model.
...
In order to search for the presence of a source, the test statistic (TS) determines whether the null hypothesis is rejected. %how favorable the alternate hypothesis is compared to the null hypothesis.
...
To convert this TS into a p-value, one has the option to simulate events using the null hypothesis probability density function multiple times.
...
In simple situations where there is only one or two degrees of freedom, the significance of a specific model can be calculated as $\sqrt{\textrm{TS}}$ according to Wilk's theorem~\cite{wilks1938}, assuming the signal model has enough events.
...
Due to time constraints, the p-value of this test statistic was not calculated with simulations, and only Wilk's theorem was used in the Crab Nebula analysis.
...
To verify the likelihood method is providing consistent results, the Crab Nebula was analyzed first, before any dark matter analysis was performed.
...
Thus, the likelihood test detects the Crab Nebula at a similar significance level, indicating this likelihood method provides results consistent with the standard analysis.
...
%Additionally, the statistical uncertainty on the background models is \nicetilde5\% though, which reduces the difference in sensitivity.
...
%A significance distribution is made of Figure~\ref{fig:crab_signif_skymap_coarse}'s pixels, and is shown in Figure~\ref{fig:crab_signif_distribution}.
%, and all bins with zero observed counts are cut, Figure~\ref{fig:crab_signif_distribution} shows that it follows a Gaussian distribution.
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.55\textwidth]{images/test_crab_analysis/plot_elev27_5_32_5deg_4_70TeV_nfits165_mapresiduals_coarse_histnozero.pdf}
...
The dark matter limits may change if these parameters are instead left free, however with more free parameters the likelihood maximization process has a tendency to converge to non-physical values and fail, so in this first-step analysis these parameters are left fixed.
...
At these high energies, the contribution from the galactic diffuse emission is negligible compared to the atmospheric effects discussed in Section~\ref{sec:elevgradient}.

There is also evidence of diffuse TeV emission around the galactic center, possibly from PeV electrons~\cite{gc_pevatron}.
The flux from this diffuse source is of a similar strength to the galactic center point source.
In the dark matter search this is unmodeled, in order to simplify the analysis.
Leaving this unmodeled will cause the background models to be fitted with slightly higher normalizations, which will reduce the number of gamma rays available for fitting the galactic center point source and dark matter models.
This has the tendency to make any dark matter upper limits more conservative.
...
% Glen Cowan presentation on upper limits
%   https://www.pp.rhul.ac.uk/~cowan/stat/nikhef/cowan_nikhef_2011_4.pdf
%
% Confidence level computation for combining searches with small statistics
%   Junk, 1999
%   https://www.sciencedirect.com/science/article/pii/S0168900299004982
%
...
\begin{figure}[bt]
  \centering
  \includegraphics[width=0.45\textwidth]{images/ulimit_diagram/ulimdiagram2.pdf}
