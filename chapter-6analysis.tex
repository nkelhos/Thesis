...
\section{Dark Matter Likelihood Analysis}\label{sec:dmlike}
  ...
  To check for any poorly modeled areas of the sky, the significance of each bin's difference between counts and models can be calculated.
  This significance is derived the following way.
  Two hypotheses are constructed, the null and the test.
  The null hypothesis is that the model alone is enough to explain the observed number of events, while the 
  {\color{red}(add derivation here!??)}
  ...
  Equation~\ref{eqn:resmap_signif} is derived in the following way.
