...
\section{Crab Nebula Likelihood Analysis}\label{sec:crab_analysis}
  ...
  \begin{figure}[!t]
    ...
    \caption[Crab Nebula Spectrum]
    {
      ...
      {\color{red}(need to fix error calc, get slides from orel??)}
      {\color{red}(check that errorbars are now sane-ly sized, in crab and gc plots??)}
    }
  ...
  \begin{figure}[p]
        ...
        \raisebox{5.4cm}{ % y position
          {
            ...
            \setlength{\fboxrule}{1pt}
            \fbox{
              \includegraphics[height=3.7cm]{images/test_crab_analysis/plot_elev27_5_32_5deg_4_70TeV_nfits165_profe_skymap_rasterized.pdf}
            }
          }
        }
      }
    ...
    \caption[Crab Nebula Profile in Energy]
    {
      ...
      {\color{red}(finish re-adding crab plots??)}
      {\color{red}(double-check profile plot residual errorbars, theyre using sqrt(n)??)}
    }
...
\section{Upper Limit}\label{upper_limit}
  ...
  The $erf^{-1}$ is the inverse error function.
