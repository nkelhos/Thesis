...
\section{Likelihood Ratio Test}\label{sec:likeratio}
  ...
  \end{equation}
  Here, $N_o$ is the flux normalization, $E_o$ is the pivot energy, and $\gamma$ is the spectral index, which can {\color{red}(can?? -orel)} all be model parameters.
...
\section{Crab Nebula Likelihood Analysis}\label{sec:crab_analysis}
  ...
  {\color{red}(why is figure \ref{fig:datapointingelevations} and table \ref{tab:observation_times} so far back?? -orel)}
...
\section{Dark Matter Likelihood Analysis}\label{sec:dmlike}
  ...
  \begin{equation}\label{eqn:brokenplaw}
    M_e(\xtrue) = M_e(\Etrue) = N_o * { \left ( \frac{\Etrue}{E_{pivot}} \right ) }^{\gamma} {e}^{-\frac{E}{E_{cutoff}}}
  \end{equation}
  ...
  This is assembled with a simplified version of Equation~\ref{eqn:dmflux} {\color{red}(its a bit confusing mentioning an old equation here, just show \ref{eqn:dmflux} and explain it?? -orel)}:
