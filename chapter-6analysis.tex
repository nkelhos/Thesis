...
\section{Background Models}\label{sec:bkgmodels}
  ...
  Because the templates are made from Sgr A* Off, which has no sources of gamma-rays, the only detectable events are due to background protons{\color{red}(qualify this statement?? -orel)}.
...
\section{Crab Nebula Likelihood Analysis}\label{sec:crab_analysis}
  ...
  This strange structuring occurs because a Gaussian distribution is symmetric about the mean, while the Poisson probability distribution is not symmetric about the mean at low averages.
...
\section{Dark Matter Likelihood Analysis}\label{sec:dmlike}
  ...
  However, this emission is not modeled, as any difference due to the missing diffuse model would be overwhelmed by the effect of the atmospheric gradient on the background models {\color{red}(dont understand this?? -orel)}.
