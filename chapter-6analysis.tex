...
\section{Crab Nebula Likelihood Analysis}\label{sec:crab_analysis}
  ...
  \begin{figure}[!t]
    ...
    \caption[Crab Nebula Profile along Galactic $l$]
    {
      ...
      The bottom plot shows the $\mathrm{All\,Models}/\mathrm{Data}$ residual as a green line, with the same blue poissonian error bars from the top plot.
      The inset plot shows the counts map from Figure~\ref{fig:crab_skymap}, with blue squares showing the profile bin locations.
      {\color{red}(redo residual to move errorbars to model line, convert model line to points, propagate error (data+-sqrt(data), and model+-sqrt(model))with orel's pdf??, theres also established binomial errors for zero data or zero model events??)}
    }
        ...
        \raisebox{6.8cm}{  % y position
          {
            ...
            \setlength{\fboxrule}{1pt}
            \fbox{
              \includegraphics[height=2.5cm]{images/test_crab_analysis/plot_elev27_5_32_5deg_4_70TeV_wobbleall_Epochall_profa_skymap_rasterized.pdf}
            }
          }
        }
      }
  ...
  \begin{figure}[!ht]
    ...
    \llap{
      \makebox[9.7cm][l]{  % x position
        \raisebox{5.6cm}{ % y position
          {
            \setlength{\fboxsep}{0pt}
            \setlength{\fboxrule}{1pt}
            \fbox{
