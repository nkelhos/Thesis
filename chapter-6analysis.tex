...
\section{Veritas Data}\label{veritasdata}
  ...
  {\color{red}(its not with the pyrometer that the clouds are warmer?? sounds strange?? -orel)}
...
\section{Likelihood Ratio Test}\label{sec:likeratio}
  ...
  {\color{red}(Formally a likelihood analysis is frequentist in nature, which means it tests the likelihood of teh data given a model (I wrote it the other way around), a statistics pedant on the committee my jump at this?? -orel)}
  ...
  {\color{red}(binned in?? binned by?? -parents)}
  When combining multiple bins arbitrarily ordered by index $j$ {\color{red}(remove arbitrarily phrase?? -orel)}, each bin's likelihood is multiplied together as in Equation~\ref{eqn:simple_like_2}.
  ...
  {\color{red}(eqn into the sentence?? -orel)}
  ...
  {\color{red}(punctuation, can also be one equation?? -orel)}
  ...
  {\color{red}(eqn into the sentence?? -orel)}
  {\color{red}(missing $m_j$?? -orel)}
  
  {\color{red}(new paragraphs?? -parents)}
  ...
  {\color{red}(incorporate equation into sentence?? -orel)}
  
  {\color{red}(please go over the whole thesis to make sure you dont begin a new paragraph after an equation like here?? -orel)}
  ...
  {\color{red}(remove E,t in equation?? -orel)}
  ...
  More complex $M_s$ functions may also have the spatial structure depend on energy $E$ or time $t$, or may take on other spatial shapes like an ellipse or a radially-symmetric profile {\color{red}(reword sentence?? -parents)}.
  {\color{red}(isnt the above equation radially symmetric?? -orel)}
  ...
  {\color{red}(remove l,b,t from function arguments?? -orel)}
  
  {\color{red}(incorporate equation into sentence?? -orel)}

  In this equation, $N_o$ is the flux normalization, $E_o$ is the pivot energy, and $\gamma$ is the spectral index, which can {\color{red}(can?? -orel)} all be model parameters.
  ...
  Another way of understanding this is if a source is located in some sky bin $j$, events from that source can be reconstructed in neighboring bins $j+1$ and $j-1$, increasing the predicted number of events in those bins and decreasing the number of events in bin $j$, as determined by the PSF {\color{red}(long sentence?? -parents)}.
  {\color{red}('as determined by the PSF' is not well connected to the beginning of the sentence?? -orel)}
  ...
  {\color{red}(why in this section you switch notation to bold text?? better stick to one notation (the previous one) -orel)}
  ...
  With Wilk's theorem~\cite{wilks1938}, the events can be repeatedly simulated using the null hypothesis probability density function {\color{red}(this is written as if wilks theorem is what allows you to repeatedly simulate events?? -orel)}.
...
\section{Background Models}\label{sec:bkgmodels}
  ...
  Background models are produced by {\color{red}binning observation sources (what do you mean?? -orel)} with weak or no gamma-ray emission.
  ...
  {\color{red}(wasnt all this explained already?? -orel)}
...
\section{Crab Nebula Likelihood Analysis}\label{sec:crab_analysis}
  ...
  After {\color{red}searching for (selecting low-elevation, not searching for?? -orel)} low-elevation Crab Nebula observations, a total of 17.1 hours of data were selected from the VERITAS data archives.
  Since the Galactic Center only rises to around \ang{30} elevation, elevation effects would also need to be searched for.
  To uncover any low-elevation effects, time cuts were applied to this data to restrict the telescope pointing elevations to \SIrange{27.5}{32.5}{\degree}, similar to the elevation of the Galactic Center data later on (see Figure~\ref{fig:datapointingelevations}).
  This resulted in \SI{3.3}{hours} of V5 and \SI{5.5}{hours} of V6 epoch data (see Table \ref{tab:observation_times}).
  {\color{red}(why is figure \ref{fig:datapoingingelevations} and table \ref{tab:observation_times} so far back?? -orel)}
  ...
  {\color{red}(invert the argument.  You limited the analysis to ar ange in which accurate IRFs are available. no need to mention computational stuff ?? -orel)}
  ...
  \begin{figure}[p]
    ...
    {
      The top plot shows the number of counts along a \ang{0.15}-wide-slice the Crab Nebula, along the galactic $l$ axis ($\ldet$ coordinate space {\color{red}(why do you mention this?? -orel)}).
  ...
  \begin{figure}[p]
    ...
    {
      The top plot shows the number of counts along a \ang{0.15}-wide-slice through the Crab Nebula along the galactic $b$ axis ($\bdet$ coordinate space {\color{red}(why mention this?? -orel)}).
      Blue points are the number of observed counts, with Poissonian error bars~\cite{poissonfrequentistinterval}.
      The green histogram bars are the number of counts predicted by all models.
      Yellow histogram bars are the number of counts predicted by only the camera-background models.
  ...
  \begin{figure}[p]
    ...
    {
      The top plot shows the number of counts along a \ang{0.15}-wide-slice along the galactic $l$ axis ($\ldet$ coordinate space {\color{red}(why mention this?? -orel)}).
  ...
  \begin{figure}[p]
    ...
    {
      The top plot shows number of counts in a \ang{0.6} x \ang{0.6} square centered on the Crab Nebula, vs energy ($\Edet$ space {\color{red}(why mention this?? -orel)}).
      Blue points are the number of observed counts, with Poissonian error bars~\cite{poissonfrequentistinterval}.
      The green histogram bars are the number of counts predicted by all models.
      Yellow histogram bars are the number of counts predicted by only the camera-background models.
      The bottom plot shows the $\mathrm{All\,Models}/\mathrm{Data}$ residual as green points.
  ...
  In Figure~\ref{fig:crab_skymap} {\color{red}(refer to figure \ref{fig:crab_skymap} before figure \ref{fig:crab_test_spectra}, either move the sentence or the figure ?? -orel)}, the position of all counts is shown in galactic $\ldet$ and $\bdet$ {\color{red}(dont bold these?? -orel)}.
  ...
  {\color{red}(need to mention figure \ref{fig:crab_profile_l_off} somewhere?? -orel)}
...
\section{Dark Matter Likelihood Analysis}\label{sec:dmlike}
  ...
  \begin{figure}[bt]
    ...
    \caption[Galactic Center Counts Skymap]{
      ...
      Note that these are in ($\ldet$,$\bdet$) coordinates, not ($\ltrue$,$\btrue$ {\color{red}(do you really need to mention this?? notice the bold notation here while before was the math mode notation?? -orel)}).
  ...
  {\color{red}(l,b,E,t notation?? -orel)}
  {\color{red}(pivot and cutoff in text mode?? -orel)}
  {\color{red}(incorporate equation into sentence?? -orel)}
  
  The {\color{red}initial values (it is an initial value only for the normalization, for the rest those are fixed values?? -orel )} used in Equation~\ref{eqn:brokenplaw} are from Ref. \cite{VeritasGCRidge2015}, where the specific values are $E_{pivot}=\SI{1}{TeV}$, $E_{cutoff}=\SI{12.8}{TeV}$, and $\gamma=-2.1$.
  ...
  This is assembled with a simplified version of Equation~\ref{eqn:dmflux} {\color{red}(its a bit confusing mentioning an old equation here, just show \ref{eqn:dmflux} and explain it?? -orel)}, shown in Equation~\ref{eqn:dmmodel}.
  ...
  Each observation has a camera background model, with two free parameters, a normalization $N_o$ and a spectral index $\gamma_o$ {\color{red}(why does it have a spectral index?? -orel)}.
  ...
  {\color{red}(maybe add a reference to the upper limit section here?? or divide or rearrange the section here?? its just that the results of no DM are lost by the time the reader gets to the upper limit section?? -orel)}
  ...
  \begin{figure}[t]
    ...
    \caption[Galactic Center Profile vs Galactic $l$]{
      ...
      {\color{red}(missing description of the bottom panel, copy from above?? -orel)}
    }
  ...
  Figure~\ref{fig:gc_profile_energy} shows the same profile {\color{red}(can I say its a similar profile?? -orel)}, except it is integrated over a \ang{0.2}$\times$\ang{0.2} galactic ($l$,$b$) square centered on Sgr A*.
  ...
  \begin{figure}[t]
    ...
    \caption[Galactic Center Profile vs Galactic $b$]{
      ...
      Modeled counts from camera background models are shown in yellow, total modeled counts are shown in green.
      Observed counts with Poissonian errors are shown in blue.
      {\color{red}(missing description of bottom panel, copy from above?? -orel)}
    }
  ...
  \begin{figure}[ht]
    ...
    \caption[Galactic Center Residual Map Coarse Binning]
    {
      ...
      The dark upper-left and lighter lower-right areas are due to the the radially-symmetric background, whereas a non-radially symmetric one would have been more ideal {\color{red}(move this sentence to the text.  should be rephrased as well(and honestly I would like to understand the problem again) ?? -orel)}.
    }
    \label{fig:gc_resmap}
  \end{figure}
...
\section{Impact of Elevation Gradient}\label{sec:elevgradient}
  ...
  \begin{figure}[ht]
    ...
    \caption[Background Gradient Comparison]{
      ...
      {\color{red}(move the last two sentences to the text?? -orel)}
    }
