...
The signficance of each sky bin is calculated with the equation
...
{\color{red}(it doesnt overlap at $\sigma=$0, -1, -3, and +3?? not sure if this is still true??)}
...
Then, each bin's significance is calculated with Equation~\ref{eqn:resmap_signif} and shown in Figure~\ref{fig:gc_resmap}.
...
However, the red and gray distributions in Figure~\ref{fig:gc_resmap_sighist_coarse} would still line up, and their residual (bottom plot) would be consistant with 1.
Because the residual is not consistant with 1, this shows there are differences between the models and the data.
...
The peaked bin around $\sigma=0$ is likely due to the bins at the edge of the Galactic Center data, where the residual approaches zero, due to the negligible VERITAS sensitivity at the camera edges..
{\color{red}(needs to match up better with the previous paragraph??)}
...
{\color{red}(is this paragraph a better explaination??)}
...
\begin{figure}[tb]
  ...
  \caption[Upper Limit Calculation]{
    Theoretical maximum likelihood as the $\vacs$ is varied (blue line).
...
As such, the 9 upper limits provided by this work do not include a width, so only upper limits at \textit{exactly} thoses masses are valid.
