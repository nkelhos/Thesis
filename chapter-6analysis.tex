...
\newcommand{\vacs}{\left \langle \sigma v \right \rangle}
...
{\color{red}(why is it very important to mention?? -orel)}
...
{\color{red}(it doesn't overlap at $\sigma=$0, -1, -3, and +3? not sure if this is still true??)}
{\color{red}(I am not sure anymore what the simulations are.  But the point is that it should look like a gaussian, which it does, so why not simply fita  gaussian and show the fit is OK?  Also, in your plot I think you only take into account the uncertainty on the simulation, but if you include the uncertainties in the data, the agreement would be better. ?? -orel)}
  
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{images/test_crab_analysis/plot_elev27_5_32_5deg_4_70TeV_nfits165_mapresiduals_coarse_hist.pdf}
  \caption[Crab Residual Bin Distribution]{
    ...
    {\color{red}(add in error bars from data??)}
  }
...
{\color{red}(is this quite true? hooper presentation at tevpa2018 said its still uncertain??)}
...
{\color{red}(do you have any cut on edge bins? what is the field of view you are considering in your analysis?? -gernot)}
{\color{red}(how does Figure~\ref{fig:gc_resmap_sighist_coarse} look like without those edge bins? is it the expected N(0,1) normal distribution?? -gernot)}
{\color{red}(but there is clearly a systematic offset in the background: where do you discuss the possible reason for that?? -gernot)}
{\color{red}(where do you discuss how all these discrepancies affect your results (ULs on DM)?? -gernot)}
...
The distribution of the pixel significances from Figure~\ref{fig:gc_resmap} is also shown in gray in Figure~\ref{fig:gc_resmap_sighist_coarse}, along with a set of 1000 simulated distributions in red.
...
{\color{red}Because only four of the residual bins are consistent with 1, this shows there are differences between the models and the data (better to write something like "differences are observed between the models and the data" ?? -orel)}.
...
Because no dark matter signal was detected, the next step is to calculate what parts of the parameter space have been ruled out.
...
To calculate the upper limit, the likelihood fitting is run with one dark matter model.
Then, $\vacs$ is increased a small amount and fixed.
The remaining free model parameters are then iteratively optimized until a new maximum likelihood is found.
...
This procedure of increasing $\vacs$, then rerunning the likelihood maximization, is repeated until the likelihood has decreased by a fixed amount, $\Delta{}\LogLike$.
The factor $\Delta{}\LogLike$ is calculated via
...
This factor is chosen because the likelihood naively follows a Gaussian distribution as any parameter is varied.
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.5\textwidth]{images/ulimit_diagram/ulimdiagram.pdf}
  \caption[Upper Limit Calculation]{
    Example maximum likelihood as the $\vacs$ is varied (blue line).
    The procedure starts at parameter $\vacs_{\mathrm{start}}$, and is increased until the likelihood has decreased by $\Delta{}\LogLike$.
  }
...
From the nine WIMP masses tested in Section~\ref{sec:dmlike}, the calculated upper limits are shown in Figure~\ref{fig:ulim}.
...
As such, the nine upper limits provided by this work do not include a width, so only upper limits at \textit{exactly} those masses are valid {\color{red}(your results are valid also for particles with a different mass but wide enough to produce a signal in the masses you tested.  That is quite important because we do expect a pretty wide WIMP at those energies, so you cover a non-negligible part of the mass spectrum.  When you write 'exactly', it means your results are sort of meaningless because it means that you wont see a DM particle with a mass of 31 TeV, which is not true ?? -orel)}.
Upper limits cannot be placed between those masses until further dark matter signals are simulated.
{\color{red}(I would instead write something like "This study can be expanded in the future to include also narrow DM particles".  Present it as an expansion of this study rather than a technical difficulty or oversight ?? -orel )}
...
Around the dark blue upper limit line from this work, systematic uncertainty error bars are also shown.
...
This can be considered representative of the \SIrange{7}{70}{\TeV} energy range, as the background does not change strongly there (see Figure~\ref{fig:background_grid}).
{\color{red}(why just this mass? does the statistics you have depend on the mass? one my worry that the effect is small for this mass, where your upper limits are not competitive anyway, but might be big for large masses where you are competitive? we don't have a good feeling for what the effect of this is at higher masses? , or can 10TeV be a good representative of 70TeV??)}
{\color{red}(I think this argument is ok.  I would just rewrite the sentence a little.  The energy range you want to say it is relevant for is above 10 TeV, so write 10-70 TeV and instead of 'change strongly there', 'strongly change in that range' ?? -orel)}
