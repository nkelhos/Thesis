...
\section{Likelihood Ratio Test}
  ...
  Individual models within a group may need to be folded with the PSF and Energy Dispersion, before the likelhood can be calculated.
  ...
  When the number of events observed in some detector bin are determined by poisson statistics, the likelihood $\Like$ is calculated by Equation~\ref{eqn:simple_like}.
  In it, $n$ is the number of observed events in the bin.
  $m$ is the number of events a model group is predicted to deposit in the bin.
  ...
  In this equation, $l$ and $b$ are Galactic sky coordinates\footnote{This thesis focuses on a galaxy-centered source, so galactic l and b are used here.  The sky coordinate system can be exchanged with an equitorial system like RA/Dec or any other spherical coordinate system without a loss of efficacy.}, $E$ is the energy, and $t$ is the time.
  ...
  The integration $\int_\xtrue$ takes place over the entire true space, time, and energy region being studied.
  Within CTOOLS, these are the Region Of Interest (ROI), the Good Time Interval (GTI), and the energy range.
  ...
  An important distinction is that this folding is only applied to these astrophysical models, and not to the camera background models\footnote{In Gammalib and CTOOLS, model class names that start with GModel* are astrophysical models that will have folding applied to them, while model class names that start with GCTAModel* are already in reconstructed space.}.
  ...
  If the likelihood has increased, these new parameter values are kept, if it descreased, then they are varied in the other direction.
  This procedure is then repeated, until no more changes in the value of the likelihood are noted.
  There are several maximization algorithms, and by default CTOOLS uses the Levenberg-Marquardt method~\cite{marquardt1963algorithm}, though other algorithms can be added.
  ...
  With Wilk's theorem, the if events are repeated simulated using the null hypothesis probability density function, the resulting TS's will form a $\Chi^2$ distribution with $n$ degrees of freedom, where $n$ is the difference in number of free parameters between the two hypotheses~\cite{wilks1938}.
  ...
  {\color{red}(is this how we convert a TS into a p-value??)}
