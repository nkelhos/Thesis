...
\section{Likelihood Ratio Test}
  ...
  $\xtrue$ is a coordinate in true space (galactic $\ltrue$ and $\btrue$, energy $\Etrue$, and time $\ttrue$) space, before folding is applied.
  On the other hand, $\xdet$ is the coordinate in reconstructed detector space ($\ldet$ and $\bdet$, $\Edet$, and $\ttrue$), after folding is applied.
  ...
  \begin{equation}\label{eqn:foldingR}
    R(\xdet,\xtrue) = A_{\textrm{eff}}(\xtrue) * PSF(\xdet,\xtrue) * E_{\textrm{disp}}(\Edet,\xtrue)
  \end{equation}
  
  In Equation~\ref{eqn:foldingR}, functions $A$, $PSF$, and $E$ are all calculated from tables of stored values, and are derived from simulations.
  ...
  Once the likelihood for a particular model is calculated, 
  ...
  \begin{equation}
    \textrm{TS} = - 2 \; \textrm{log} \left (  \frac{ \LogLike(\hat{d},M_{\textrm{null}}) }{ \LogLike( \hat{d}, M_{\textrm{alt} } ) } \right )
  \end{equation}
  ...
  The statistical preference for one model over the other is called the Test Statistic (TS), which is calculated in Equation~\ref{eqn:test_statistic}.
  
  % from https://arxiv.org/pdf/1606.00393.pdf, equation 50
  \begin{equation}\label{eqn:test_statistic}
    \text{TS} = - 2 \textrm{log} \left ( \frac{ \Like \left ( \hat{d}, \, M_{\textrm{null}} \right ) }{ \Like \left ( \hat{d}, \, M_{\textrm{alt}} \right ) } \right )
  \end{equation}
  
  % from https://arxiv.org/pdf/1606.00393.pdf, equation 6
  \begin{equation}\label{eqn:sum_likelihood}
    \Like \left( \hat{d}, M \right ) = \sum_i \textrm{log} \, \textrm{P} \left( M, d_i \right )
  \end{equation}
...
\section{Crab Nebula Analysis}\label{sec:crab_analysis}
  ...
  {\color{red}(State Crab model as a function of $M(\xtrue)$ !??)}

  The sky position of the point source and the pivot energy $E_{0}$ of its spectrum were fixed, while the prefactor $I_{0}$ and the spectral index $\gamma$ were free to vary during the likelihood optimization.
