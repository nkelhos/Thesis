...
\section{Veritas Data}\label{veritasdata}
  ...
  With the pyrometer, clouds are significantly warmer (~\ang{50} C) than clear sky.
  Low-quality segments of data, where there are large (>20\%) and/or rapid (<=\SI{30}{s}) changes in the L3 trigger rate or cloud ceiling height, are removed from the analysis.
  {\color{red}(find weather quality cuts citation!?? -Orel)}
  % see Bird 2015 thesis pg 55

  Because VERITAS epoch has a different hardware configuration, each epoch has its own separate set of effective areas, point spread functions, energy migration matricies, and camera background models.
...
\section{Likelihood Ratio Test}
  The likelihood ratio test determines which of two model groups is a statistically better fit to some data.
  It is performed by calculating, for a group of events, the likelihood of detecting that group of events with each of two model groups.
  ...
  These two maximum likelihoods can then be used to calculate the Test Statistic, which determines which model is statistically favored, and to what degree it is favored.
  ...
  In it, $n$ is the number of observed events, and $m$ is the number of events a model group predicts.
  
  \begin{equation}\label{eqn:simple_like}
    \Like = \frac{e^{-m} m^n}{n!}
  \end{equation}
  
  The average number of events $m$ is determined by integrating over a group's models in energy, sky position, and time, after folding in the PSF and the energy dispersion.
  {\color{red}(It is not found by integrating over the group's model, but by maximizing the likelihood!?? -orel)}
  ...
  To preserve the test's power, the above result can be expanded into an unbinned likelihood through the following derivation.
  ...
  $m_j$ converts to $m_i$, because all the $n=0$ bins are now 1 and can be ignored, so a loop over the $j$ bins becomes a loop over the $i$ events.
  ...
  When the bin size is infinitly small, $m_i$ becomes the value of the probability density function (of all models combined) at the position of the event, $P_i$, as in Equation~\ref{eqn:simple_like_8}.
  ...
  Equation~\ref{eqn:simple_like_8} is how the unbinned log-likelihood used in this analysis is calculated.
  The sum over $m_j$ can be interpreted as the total number of events predicted by all models, sometimes referred to as $N_{pred}$.
  The reader should note however, that while a binned likelihood calculation scales with the number of bins (and some lost information), the unbinned likelihood calculation scales with the number of events, which can lead to large likelihood maximization times (see later in this section).
  ...
  More complex $M_s$ functions may have the spatial structure also depend on energy $E$ or time $t$, or may take on other spatial shapes like an ellipse or a radially-symmetric profile.
  ...
  $m_k$ is number of counts predicted by model $k$, at the position of event $i$ or bin $j$.
  ...
  {\color{red}(why does three observations = 3 background models plus 1 source model?? -orel)}
  ...
  The Crab Nebula model also has its location parameters $l_c$ and $b_c$, but since these are fixed in this example (and thesis), the likelihood maximization won't try to vary them.
  ...
  When finding the maximum likelihood for each hypothesis, these free parameters are incrementally varied, and the likelihood is recalculated, repeated until a maximum likelihood is reached.
  ...
  For searching for the presence of a source, the Test Statistic (TS) determines how favorable the alternate hypothesis is compared to the null hypothesis.
  ...
  With Wilk's theorem, the events can be repeatedly simulated using the null hypothesis probability density function.
  For each simulation, a TS can be calculated.
  After many simulations, resulting TS's will form a $\chi^2$ distribution with $n$ degrees of freedom, where $n$ is the difference in number of free parameters between the two hypotheses~\cite{wilks1938}.
  From this distribution, a TS can be converted into a p-value.
  In situations where there is only one or two degrees of freedom {\color{red}(??)}, the significance of a specific model can be calculated as $\sqrt{\textrm{TS}}$.
  {\color{red}(cleanup this paragraph more??)}
  
  {\color{red}(is this how we convert a TS into a p-value?? discuss with orel and galo more)}
...
\section{Crab Nebula Analysis}\label{sec:crab_analysis}
  ...
  The Crab was modeled by a point source with the simple power law spectrum in Equation~\ref{eqn:crab_model}, both of which are discussed in Section~\ref{subsec:likemax}.
  ...
  In order to ensure there were enough simulations to properly populate the high-energy end of the IRFs, the analysis was limited to a maximum event energy of \SI{70}{TeV}.
  ...
  After fitting all model parameters to the events from \SIrange{4}{70}{TeV}, the best fit power law values were $ N_o = \left(3.90\pm0.71\right)*10^{-20} \frac{\textrm{photons}}{\textrm{cm}^{2} \; \textrm{s} \; \textrm{MeV} } $, $ \gamma = 2.31 \pm 0.17 $, with a Test Statistic of 408.8, corresponding to \SI{~20}{$\sigma$}.
  ...
  \begin{figure}[h]
    ...
    {
      The number of counts along a \ang{0.16}-wide-slice through the Crab along the galactic l axis ($\ldet$ coordinate space).
...
\section{Dark Matter Likelihood Analysis}\label{sec:dmlike}
  ...
  As the test analysis on the Crab Nebula data shows results consistant with other VERITAS, H.E.S.S., and MAGIC studies, the main dark matter analysis can begin in earnest.
  ...
  {\color{red}(is there value to showing these two uncorrected histograms?? -orel)}
  Both of these plots have no corrections for observing time or effective area.
  After gathering the data, assembling the models is the next step.
  ...
  {\color{red}(comma between each citation?? -Orel)}
  Instead, a point source with the broken power law spectrum in Equation~\ref{eqn:brokenplaw} was added to the list of models.
  {\color{red}(why a broken power law?? it should be motivated since its not the simplest model -Orel)}
  ...
  The initial values used in Equation~\ref{eqn:brokenplaw} are from \cite{VeritasGCRidge2015}, where the specific values are $E_{pivot}=\SI{1}{TeV}$, $E_{cutoff}=\SI{12.8}{TeV}$, and $\gamma=-2.1$.
  % theres also the 2016 paper http://iopscience.iop.org/article/10.3847/0004-637X/821/2/129/meta
  The normalization parameter $N_o$ was initially set to $2.8*{10}^{-12}\,\text{cm}^{-2}\,\text{s}^{-1}\,\text{TeV}^{-1}$, but was left free in the likelihood optimization, while $E_{pivot}$, $E_{cutoff}$, and $\gamma$ were all fixed.
  ...
  See Section~\ref{dm_spatial} for a discussion on this $M_{s,\textrm{halo}}$ function.
  ...
  From Wilk's theorem~\cite{wilks1938}, with a TS value greater than 20 would hint at the presence of a dark matter halo.
  ...
  \begin{table}
    ...
    \caption[DM Halo TS Values]{
      ...
      {\color{red}(this table should come after the paragraph that references it!??)}
    }
    \label{tab:tsvals}
  \end{table}
  ...
  \begin{figure}[ht]
    ...
    \caption[Galactic Center Residual Histogram]
    {
      ...
      {\color{red}(Describe what the green line is?? -orel)}
    }
...
\section{Upper Limit}
  From the dark matter flux Equation~\ref{eqn:dmflux} {\color{red}(repeat the equation here?? -orel)}, an upper limit on the observed gamma-ray flux can be derived.
...
\section{Impact of Elevation Gradient}\label{sec:elevgradient}
  ...
  \begin{figure}[ht]
    ...
    \caption[Background Gradient Comparison]{
      ...
      {\color{red}(orel can barely see a difference, any way to make it clearer??)}
    }
  ...
  Thus, the likelihoods from both model groups changed by a similar amount, and in the same direction, partially cancelling.
