...
\section{Likelihood Ratio Test}
  ...
  \begin{equation}\label{eqn:unbinned_likelihood}
    - \text{ln} \, L_i \left ( M \right ) = e_i \left ( M \right ) - \sum_k \text{ln} \, P_i \left ( p'_k, E'_k, t'_k \, | \, M \right )
  \end{equation}
  
  Where $i$ is the $i^{\text{th}}$ observation, also referred to as a chunk in Section \ref{fitsconversion}.
  $k$ is the $k^{\text{th}}$ event in the observation, $p'_k$ is the event point-of-origin, $E'_k$ is the event energy, and $t'_k$ is the event arrival time.
  The function $P$ is the poissonian probability of getting an event with properties $p'$, $E'$, and $t'$, given the hypothesis $M$, shown in Equation \ref{eqn:probability_summation}.
  
  \begin{equation}\label{eqn:probability_summation}
  P \left ( p, E, t | M \right ) = \sum_j P_i \left ( p, E, t | M_j \right )
  \end{equation}

  {\color{red} need to fix equation \ref{eqn:probability_summation}, uses two sets of p,E,t: detector and sky coordinate systems!}
  Where $M_j$ is the $j^{\text{th}}$ model within the hypothesis $M$.
  ...
  $P_i \left( p, E, t | M_j \right )$ is the probability of getting an event from a single model $M_j$, depicted in Equation \ref{eqn:single_probability}.
  
  % from https://arxiv.org/pdf/1606.00393.pdf, equation 8
  \begin{equation}\label{eqn:single_probability}
    P_i \left ( p, E, t | M \right ) = \int_{p,E,t} R_i \left( p,E,t|p,E,t \right ) \times M_j^S \left( p, E, t \right ) dp dE dt
  \end{equation}
  {\color{red} need to fix equation \ref{eqn:single_probability}, uses two sets of p,E,t: detector and sky coordinate systems!}
  ...
  \begin{equation}\label{eqn:model_integration}
    e_i \left(M \right ) = \int_{GTI} \int_{Ebounds} \int_{ROI} P_i \, \left( p', E', t' \, | \, M \right ) dp' \, dE' \, dt'
  \end{equation}
  ...
  \subsection{Test Crab Analysis}
    ...
    \begin{figure}[h]
      ...
      \caption[Crab Counts Skymap]
      {
        ...
        {\color{red}Observation time is different??}
      }
...
\section{Dark Matter Likelihood Analysis}
  ...
  This is because other analyses {\color{red}(cite??)} have indicated that the bright gamma-ray source at Sgr A* is not best modeled by a dark matter halo.
  ...
  For the spectral component, each dark matter mass tested has its own spectrum produced with clumpy, which is {\color{red}based on ??} (see section \ref{dm_spectral}).
