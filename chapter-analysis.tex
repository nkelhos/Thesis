...
\section{Likelihood Ratio Test}
  ...
  When combining multiple bins ordered by index $j$, each bin's likelihood is multiplied together as in Equation~\ref{eqn:simple_like_2}.
  ...
  The above result can be expanded into an unbinned likelihood through the following derivation.
  ...
  {\color{red}(mention that $\prod_i m_i$ encodes the positions of the events, this is the data, whereas the $\sum_j$ is the models??)}
  ...
  {\color{red}(describe $P_i$ earlier, or transition from $m_i$ to $P_i$ right after $m_i$ is defined)}
  ...
  After many simulations, resulting TS's will form a $\chi^2$ distribution with $n$ degrees of freedom, where $n$ is the difference in number of free parameters between the two hypotheses.
  ...
  {\color{red}(explain that there wasn't time to calculate the actual P-value)}
...
\section{Background Models}\label{sec:bkgmodels}
  ...
  {\color{red}(Explicitly say that these background templates are used in both the crab analysis and the GC, and mention why this decently motivated (and ok to do) (normal background for point sources are made by subtracting the source and binning radially ??)}
...
\section{Crab Nebula Likelihood Analysis}\label{sec:crab_analysis}
  ...
  \begin{figure}[h]
    ...
    \caption[Crab Test Spectrum]
    {
      ...
      {\color{red}(Remove all but 1 CTOOLS reference!!! in the whole thing!!!!!!)}
...
\section{Dark Matter Likelihood Analysis}\label{sec:dmlike}
  ...
  {\color{red}(mention that the lack of corrections explains the weird overlapping shape??)}
  A histogram of all events' energies from \SIrange{4}{70}{TeV} is shown in Figure~\ref{fig:gc_counts_enhist}.
  {\color{red}(mention that the lack of corrections explains why it doesn't look like a power law??)}
  ...
  {\color{red}(only got to here with orel's comments!??)}
