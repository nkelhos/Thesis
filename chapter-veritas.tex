...
\begin{figure}[ht]
  ...
  \caption[Polaris PSF]{
    ...
    {\color{red}(do you have a 2d plot of OPSF?? -maria)}
...
If the low-gain trigger pulse was also recieved by the FADC, then the signal voltage pulse directed to a separate circuit that does not amplify the voltage pulse as much before digitizing it.
...
This was done because the R10560 has a considerably higher collection rate of Chernekov photons (35\%), compared to the XP2970 (23\%).
...
{\color{red}Once every few nights (every few nights or once per dark run?? -maria)} the single-photoelectron curves for each PMT are measured.
...
These requirements include that multiple L1 triggers must fall within a certain time window, and that multiple L1 triggers match a pattern condition.
