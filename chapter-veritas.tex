...
This is accounted for by separate simulations for each PMT model, mostly resulting in different effective areas at the lower energies.(check!??)
...
As these different epochs have different telescope configurations, the instrument response functions are different, effectivly meaning each epoch is a measurably distinct observatory.
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption[Angular Reconstruction Disp]{The disp parameter is the angular distance between the center (red dot) of a hillas image (blue oval) and the reconstructed sky position (green dot).  Generally, longer showers have a larger disp angle, while shorter showers have a shorter disp angle.}\label{fig:dispdiagram}
  \end{center}
\end{figure}
