...
This combined pulse then sent to a Zero Crossing Discriminator (ZCD).
...
In order to reduce this contamination, the CFD circuit is set to ignore any pulses smaller than 45mV.
...
If the low-gain trigger pulse was also recieved by the FADC, then the signal voltage pulse directed to a separate low-gain circuit that does not amplify the voltage pulse before digitizing it.
...
Once the voltage pulse is digitized, it is saved to an \SI{8}{$\mu$s}-long rolling buffer in the FADC, waiting for higher level triggers, signaling the FADC to readout its buffers into a datafile~\cite{Veritas_Detector,veritas_FADC}.
...
The L2 (image) trigger is emitted when a group of L1 triggers meet certain requirements.
These requirements are that multiple L1 triggers must fall within a certain time window, and that multiple L1 triggers match a pattern condition.
Multiple L1 triggers must be within \SIrange{5}{10}{ns} of each other, depending on the VERITAS hardware epoch.
Any pixel with an L1 trigger and two neighboring pixels with L1 triggers (each pixel has 6 neighboring pixels) meets the pattern condition~\cite{veritas_v6_trigger}.
...
In 2007, the fourth telescope was constructed at (-35.9n, 11.3e), and data taken between this point in time and the next major upgrade is considered the fourth epoch (V4).
