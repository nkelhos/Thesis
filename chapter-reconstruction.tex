...
\section{Pedestal Variation}
  ...
  From this pedestal, the pedestal variations are then calculated as the rms of the all the dc counts in all noise-events, which can be visualized as the distribution of dc around the mean dc.
...
\section{Position Reconstruction}\label{subsec:posrecon}
  ...
  \subsection{Angular Reconstruction Boosted Decision Trees}\label{subsec:disp}
  ...
  % Once trained on these parameters, the machine learning algorithm can construct a boosted decision tree for determining any new image's \textit{disp}, which can then be used with the image axes intersections to improve the gamma-ray reconstructed positions.
    ...
    These parameters for thousands of simulated showers can then be used to train a boosted decision tree forest (BDT) that estimates the \disp{} for a new shower's images.
...
\section{Energy Reconstruction}\label{subsec:enrecon}
  After each events' images are quantified, its properties are then used with a lookup table to find the energy of each shower.
  {\color{red} expand??!}
...
\section{FITS Conversion for Gammalib and CTOOLs}\label{fitsconversion}
  ...
  The point spread function (sometimes referred to as psf) quantifies the distribution of true positions for a reconstructed position, which is needed for identifying point sources and extended spatial structures.
  ...
  \subsection{Background Models}\label{background_production}
    ...
    This is used to quantify how many counts are expected when observing a gamma-ray-dark sky position, which estimates how many background events are being detected at different energies and in different parts of the camera.
  ...
  \subsection{Energy Dispersion}\label{subsec:edisp}
    ...
    In an analysis with CTOOLS, this \textit{dispersion in energy} is quantified by an energy migration matrix $E_{i,j}$, where $i$ denotes the $i^{\text{th}}$ reconstructed energy bin, and $j$ denotes the $j^{\text{th}}$ true energy bin.
...
\section{Camera Studies}
  ...
  \subsection{Effect of Stars}
    ...
    To examine the effects of disabled pixels, \nicetidle13 hours of Crab observations were analyzed twice with Eventdisplay.
