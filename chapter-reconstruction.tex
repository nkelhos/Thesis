  ...
  % Once trained on these parameters, the machine learning algorithm can construct a boosted decision tree for determining any new image's \textit{disp}, which can then be used with the image axes intersections to improve the gamma-ray reconstructed positions.
    ...
    \begin{figure}[ht]
      ...
      \caption[DISP Offset Improvement]{
        ...
        {\color{red} (Normalize the Y axis to the total integral??)}
...
\section{FITS Conversion for Gammalib and CTOOLs}\label{fitsconversion}
  ...
  To allow for the inclusion of this time dependence in the analysis, an alternate method of storing the IRFs was chosen, diagrammed in Figure \ref{fig:fits_scheme} {\color{red} (this physics justification is good, but all the other software discussion should be removed)}.
  ...
  \subsection{Background Models}\label{background_production}
    ...
    {\color{red}(Each background model is constructed from one of two templates, and the templates are built from dark run observations??)}
    {\color{red}Any chance you write something like camera x-y coordinate system where x is [...] and y is [...]? Took me a second to understand what you meant (partly because x and y were capital.?? -Orel}.
    {\color{red}(use lowercase x and y for these camera coordinates! ??)}
    These models calculate the number of expected events in the camera, per unit solid angle, unit time, and unit energy.
    This is used to {\color{red}quantify how many counts are expected in different parts of the camera when observing any target.
  ...
  \subsection{Energy Dispersion}\label{subsec:edisp}
    ...
    {\color{red}(Proofread the following sentences!??)}
    ...
    Multiple {\color{red}simulations (Change to discussing real data (photons with the same energy will be measured with a distribution about that energy, etc.  Otherwise it sounds like this randomization happens only in sims.)?? -Orel)} of the same energy event will result in a distribution of reconstructed energies.
...
\section{Camera Studies}
  ...
  \subsection{Background Structure at the Low Energy Threshold}
    ...
    In Figure \ref{fig:back_lowelev29} and Figure \ref{fig:back_lowelev26}, the same plots are constructed for a set of low-elevation (\nicetilde{}\ang{29} and \nicetilde{}\ang{26} respectivly) observations, using Galactic Center Off observations {\color{red}(Did you explain what this means?? -Orel) (should be switched to 'dark runs' or something}.
  ...
  This \nicetilde20\% higher airmass causes air showers to start further away from the telescopes {\color{red}(also mention increased cherenkov photon absorption??)}, resulting in fewer events in the lower half of the camera have Cherenkov photons actually reach the camera.
  ...
  Radial backgrounds (sometimes referred to as acceptances) are typically used in VERITAS as they require less data to construct {\color{red}(maybe even remove this 'less data' feature)}, and no other Camera X/Y or energy dependence had been demonstrated until now.
...
I am sure that if you do this calculation you can estimate that the effect is much smaller than the statistical/systematic uncertainties and then it is justified to neglect. ?? -Orel}
    ...
    \begin{figure}[ht]
      ...
      \caption[Radial Relative Event Rate]{
        ...
        {\color{red}(Calculate these relative event rates based on only events that appeared both with and without the pixel, so that the max rate (and its errorbars) never go above 1. ??)}
      }
