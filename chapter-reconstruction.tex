  ...
  % Once trained on these parameters, the machine learning algorithm can construct a boosted decision tree for determining any new image's \textit{disp}, which can then be used with the image axes intersections to improve the gamma-ray reconstructed positions.
    ...
    \begin{figure}[ht]
      ...
      \caption[DISP Offset Improvement]{
        ...
        {\color{red} (Why does the crab see so little benefit??  why does the mean disp move left in the crab, but right in Sgr A*?) }
      }
...
\section{FITS Conversion for Gammalib and CTOOLs}\label{fitsconversion}
  ...
  To allow for the inclusion of this time dependence in the analysis, an alternate method of storing the IRFs was chosen, diagrammed in Figure \ref{fig:fits_scheme}.
  ...
  \subsection{Effective Area}\label{subsec:effarea}
    ...
    For each point in the parameter space, the effective areas are calculated with many {\color{red}(how many?? -Gernot)} Monte Carlo gamma ray simulations.
    ...
    \begin{figure}[ht]
      ...
      \caption[Effective Area Parameter Space]{
        ...
        Some event positions from the run are shown as black dots, indicating which parts of the parameter space are used more than others.
        {\color{red}If I understand correctly, the effective area shown is calculated from simulation, but the points are from data? Can you please clarify in the caption?? -Orel}
      }
    ...
    \begin{figure}[ht]
      ...
      {\color{red}I don't get what we learn from these graphs?? -Gernot}
      }
  ...
  \subsection{Point Spread Function}\label{subsec:psf}
    ...
    {\color{red}The same gamma ray may develop into a different shower (with different particles early in the shower gaining different amounts of energy or scattering at different angles), or different shower Cherenkov photons may be absorbed by the atmosphere, or reflected by the mirrors, or converted into PMT photoelectrons.
    Can you please divide this long sentence to a few short ones?? -Orel}
    ...
    With VERITAS and Eventdisplay, a gaussian function was chosen to fit the psf distribution.
    For CTOOLS however, the distribution of events are instead fitted with a King function \cite{king1962} (see eqn \ref{eqn:king}), as the Fermi LAT team has found gamma ray psfs tend to have longer tails than a gaussian \cite{fermipsf}, which the King function better handles {\color{red}Why does Fermi matter for VERITAS?? -Gernot}.
    {\color{red} Can you write this without the specifics of software constraints? If King function describes gamma-ray PSF better, it is a good enough reason to use it. You actually do not even have to mention what is usually done in VERITAS. (??) -Orel}
    ...
    \begin{figure}[ht]
      \centering
      \includegraphics[width=0.9\textwidth]{images/psf_king_plots/psf_parameter_space.eps}
      \caption[PSF Parameter Space]{
        ...
        {\color{red}If I understand correctly, the PSF shown is calculated from simulation, but the points are from data? Can you please clarify in the caption?? -Orel}
        {\color{red}Use a more visible point color, that green is too dark??}
      }
    ...
    \begin{figure}[ht]
      \centering
      \includegraphics[width=0.9\textwidth]{images/psf_gc_eventhist/eventpsf.eps}
  ...
  \subsection{Background Models}\label{background_production}
    A background model is a 3 dimensional function in camera X (in angle, parallel to azimuth), camera Y (in angle, parallel to elevation), and energy {\color{red}Any chance you write something like camera x-y coordinate system where x is […] and y is […]? Took me a second to understand what you meant (partly because x and y were capital.?? -Orel}.
    The function calculates the number of observed events in the camera, per unit solid angle, unit time, and unit energy.
    This is used to {\color{red}quantify how many counts are expected when observing a gamma-ray-dark sky position (Don’t you use data from gamma-ray-dark sky position to build the model? I don’t really understand this whole sentence.)}, which estimates how many background events are being detected at different energies and in different parts of the camera.
    ...
    {\color{red}What do you mean by interpolating in linear log? In fact, there is no need to mention histogram bins or interpolation here. ?? -Orel}
  ...
  \subsection{Energy Dispersion}\label{subsec:edisp}
    As events are reconstructed imperfectly, it is important to understand what the distribution of true energies are for a given reconstructed energy.{\color{red}Again, not sure, but perhaps the other way around?? -Orel}
    ...
    {\color{red}(Would these give you a distribution as you write or the average value for each bin i or j as 5.4 and 5.5 suggest?? -orel).}
    ...
    By unfolding {\color{red}(This is not unfolding. In unfolding, you use the migration matrix to correct a  different distribution for the effects you describe later.)} these distributions from the reconstructed energies, two significant effects are accounted for.
    The first is that the reconstruction method introduces biases in the event energy, meaning on average, an event at a given true energy is reconstructed at a higher or lower energy.
    The second effect that is accounted for is the dispersion in the reconstructed energies.
    Multiple {\color{red}simulations (Change to discussing real data (photons with the same energy will be measured with a distribution about that energy, etc.)?? -Orel)} of the same energy event will result in a distribution of reconstructed energies.
    This has the effect of smearing out events in each energy bin of a spectra.
    In astrophysical spectra, which often follow a power law, lower energy bins tend to have more events than the next highest energy bin, so smearing out the lower-energy bin contributes more to the higher-energy bin than the higher-energy bin contributes to the lower-energy bin.
...
\section{Camera Studies}
  ...
  {\color{red}(That is not the objective of the analysis though. The objective is to search for dark matter, not to admit defeat from the get go and aim at setting an upper limit.?? -Orel}
  ...
  \subsection{Background Structure at the Low Energy Threshold}
    ...
    {\color{red}(Were these effects noted because of the binning you describe or did you bin the data in order to study these effects? You should clarify. ?? -Orel)}
    ...
    In Figure \ref{fig:back_lowelev29} and Figure \ref{fig:back_lowelev26}, the same plots are constructed for a set of low-elevation (\nicetilde{}\ang{29} and \nicetilde{}\ang{26} respectivly) observations, using Galactic Center Off observations {\color{red}(Did you explain what this means?? -Orel)}.
  ...
  Due to the time-dependent rotation {\color{red}(Time dependent rotation of what?? -Orel)}, this deficit occurs in the bottom of the camera, where the atmosphere column density ($g/cm^2$) is \nicetilde20\% higher than at the top of the camera.
  ...
  {\color{red}Please reverse the order of these sentences, the lack of dependence is more important than the fact they require less data.}
  ...
  \subsection{Diffuse Simulations}\label{subsubsec:diffusesims}
    To rule out any software causes {\color{red}(You should not mention software. (also it is not really testing the software, you are using simulation where you have full control in order to see if you observe the same effect))?? -Orel}, diffuse simulations were performed at similar energies and elevations.
    ...
    These remaining events are then binned in camera coordinates in Figure \ref{fig:back_simdiffuse}.
  ...
  \subsection{Effect of Stars}
    ...
    This relative event rate is calculated by taking, {\color{red}for each bin (Bin of what? I guess what you do is reconstruct the position of the shower on the camera and divide those positions into bins? 
This description could be a little clearer. ?? -Orel)}, the number of reconstructed gamma-like events, divided by the pixel-enabled number of reconstructed events.
    ...
    {\color{red}
    I stand by my comments on the regions of the plot and of the errors. Estimating them from 1 sigma spread leads to overestimating them. You have to use binomial error calculation (success/failure) or take into account repeated events in the error calculation. Neither of these is very hard to do. Depending on which method you use, write here accordingly something like “taking into account statistical correlations between the two samples”. ?? -Orel}
...
I am sure that if you do this calculation you can estimate that the effect is much smaller than the statistical/systematic uncertainties and then it is justified to neglect. ?? -Orel}
    ...
    \begin{figure}[ht]
      \centering
      \includegraphics[width=0.85\textwidth]{images/disabled_pixel/relativerate_radial}
      \caption[Radial Relative Event Rate]{
        ...
        {\color{red}How did you calculate the error bars?? Can they really go above 1?? -Gernot}
      }
    ...
    It should be noted that relatively few (\nicetilde{}100 in the 20-minute-long run 53703 {\color{red}How many events are there in a 20 min run? People outside of VERITAS (and me…) do not know the event rate so 100 can be 10\% or 0.1\% } ) events move more than this, and the events that do move are mostly ones with non-compact image shapes that are amputated when a pixel is disabled.
