...
First, one observation (typically 20-30 minutes long) is broken up into ~3 minute chunks.
...
The end result is that a 30 minute observation can be broken into ~10 FITS files (called chunks), where each file is independent of any outside references, contains all IRF needed information, and only takes up ~100s KB.
This makes it ideal for a program to automatically download any needed data over an internet connection, as each chunk has a small filesize and has no outside dependencies.
...
There were minor issues with converting the IRFs that should be noted.
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.75\textwidth]{images/FITS_diagrams.alternate_scheme.eps}
    \caption[FITS Background at High Elevations]{}\label{fig:back_highelev}
  \end{center}
\end{figure}
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.75\textwidth]{images/FITS_diagrams.alternate_scheme.eps}
    \caption[FITS Background at Low Elevations]{}\label{fig:back_lowelev}
  \end{center}
\end{figure}
