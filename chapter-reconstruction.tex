...
\section{FITS Conversion for Gammalib and CTOOLs}\label{fitsconversion}
  ...
  To allow for the inclusion of this time dependence in the analysis, observing runs were broken up into smaller temporal parts, and each time part had IRFs assigned to each chunk based on that chunk's position in the elevation/azimuth/night-sky-background-noise parameter space.
...
\section{Camera Studies}
  ...
  \subsection{Background Structure at the Low Energy Threshold}\label{subsec:bkgstructure}
    ...
    \begin{figure}[ht]
      ...
      \caption[FITS Background at \ang{50} Elevation]{
        ...
        Notice the left-most bin (2\%-34\%) overlaps the peak (at -0.5) in the energy histogram by \nicetilde 30\% of its width, unlike in Figure~\ref{fig:back_lowelev29}.
      }
  ...
  {\color{red} Discuss how this can be used to calculate the 'detectable depth' of gamma rays in g/$\textrm{cm}^2$, and how this depth changes with energy??}
