...
\section{Production of TeV Gamma Rays}
  ...
  At Earth, astrophysical electrons have been detected at energies of \SI{500}{GeV}~\cite{500GeVElectrons,fermi_electron}, and potentially as high as \SI{20}{TeV}~\cite{hess2017_electronspectrum}.
  ...
  These expanding particles {\color{red}bring their own magnetic fields (how do they bring their own magnetic fields??  I think alfven's theorem, but I'm not sure)}, which reflect charged particles.
  ...
  The X ends up posessing only a small amount of energy compared to the mass of the pions.
  ...
  \subsection{Dark Matter Interactions}\label{dmgammaproduction}
    ...
    Another type are cryogenic detectors, which use pucks of germanium and silicon to measure WIMP collisions.
    ...
    A third example are scintillation detectors, which are built with crystals like titanium-doped sodium iodine.
...
\section{Galactic Center}
  ...
  Around this black hole, gamma ray emission is observed~\cite{gc_pointsrc_hess,gc_pointsource_hess2,gc_veritas_pointsource,gc_magic_pointsource}, though the production mechanism is still under debate.
...
\section{Indirect Dark Matter Search}
  ...
  %the radially-dependent structure of dark matter halos also affects the gamma-ray emission rate.
  %{\color{red}(I suggest after the comma "the gamma-ray emission rate is affected by the radially-dependent structure of dark-matter halos." ?? -orel)}
  ...
  \subsection{Dark Matter Halo Structure}\label{dm_spatial}
    ...
    Specifically, if the true dark matter halo has a cored profile, but is modeled using a cuspy one, then any derived upper limits on the dark matter cross section would be different.
...
\section{Atmospheric Showers}\label{sec:atmoshowers}
  ...
  The proton-proton collision then then may producea set of pions, 
  
  \begin{equation}\label{eqn:protoncollision}
    p_{\textrm{cosmic}} + p_{\textrm{atmo}} \rightarrow \pi^+ + \pi^+ + \pi^0 + X \;\;,
  \end{equation}
  
  {\color{red}(remove equation, and replace $p_\textrm{atmo}$ with nucleon N??)}
  
  as well as other particles (`$X$') whose production rates vary with available interaction energy.
  ...
  The initial energies were chosen as such because a \SI{1}{TeV} proton will on {\color{red}average divest $\frac{1}{3}$ of its energy to the \pio{} (I don't think that 1/3 of the initial energy goes to one pi0?? -orel)}.
  {\color{red}(I don't think it is true that in the first interaction a third of the energy goes to the $\pi^0$ and that is how you get the ratio 1/3 between photons and protons. I think that on average, after all of the initial proton interactions, a 1/3 of it's energy was transferred to many $\pi^0$s. That is why at higher energies that ratio changes to ~1/2.
 ?? -orel)}
