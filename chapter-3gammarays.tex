...
\section{Indirect Dark Matter Search}
  ...
  \subsection{Spectrum of Gamma Rays from WIMP Annihilations}\label{dm_spectral}
    ...
    This is done by repeatedly simulating all the photons produced when the $b$ quarks hadronize and decay into other particles.
    Several of these decays are shown in Table~\ref{tab:bpaths2}.
    In the table, the \Pbottom hadronizes into \PBplus or \PBzero, which then decay into \PK and \PD mesons (plus some other particles $X$), and the \PD mesons also decay into \PK mesons.
    The \PK mesons can then decay into \Ppipm and \Ppizero mesons, and then the \Ppizero{}'s decay into two photons.
    {\color{red}(previous two sentences??)}
