...
\section{Indirect Dark Matter Search}
  ...
  \subsection{Spectrum of Gamma Rays from WIMP Annihilations}\label{dm_spectral}
    ...
    {\color{red}As WIMP properties are not well understood (not because they are not well understood, you search for everything physically allowed ?? -orel)}, all particle-antiparticle pairs can be considered as possible annihilation channels.
    In addition, {\color{red}WIMP annihilations may have branching ratios (thats not really branching ratios, the branching ratio is for the decay, not the annihilations ?? -orel)}, where a population of WIMPs will annihilate into different channels with different probabilities.
    As a made up example, for 100 WIMP annihilations, 80 may annihilate into $b\bar{b}$, while the remaining 20 annihilate into a gluon-gluon $gg$ pair {\color{red}(orel crossed this sentence out??)}.
    However, branching ratios are beyond the scope of this analysis, so only the $100\% \rightarrow b\bar{b}$ annihilation channel is considered {\color{red}(you don't need to say this and I dont think it is actually true (if it is, we have a problem with the upper limits). Just write that you focus on the $b\bar{b}$ channel ?? -orel)}.
    ...
    The particle $\tilde{f}$ is a supersymmetric fermion, the $Z$ is the Z boson, and $A$ is the pseudoscalar Higgs particle.
    ...
    \begin{figure}[h]
      ...
      \caption[WIMP Annihilation Feynman Diagrams]{
        ...
        {\color{red}(is the found Higgs a pseudoscalar, or should I just replace the $A$ with an $H$??)}
        {\color{red}(the higgs is not a pseudoscalar, it is a scalar.  You can replace the A with an H if you want, it should still work.  Please note the comments on the paragraph above as well?? -orel)}
      }
      \label{fig:neutralino_feynman}
    \end{figure}

    After the desired channel is selected, any {\color{red}bremsstrahlung radiation (you wont get bremsstrahlung from the quarks (at least not photons) ?? -orel)} from the initial $b$ and $\bar{b}$ particles {\color{red}must be calculated (what does it mean, 'must be calculated'?? -orel)}.
    ...
    In the first three rows, a $\bar{b}$ quark decays into a $\bar{D}^0$ meson {\color{red}(b quark does not decay to D?? -orel)} and some other particles in 59.8\% of all $\bar{b}$ decays.
    The $\bar{D}^0$ then decays 54.7\% of the time into a $K^+$ and some other particles, and then finally the $K^+$ decays into a $\mu^+ + \nu_{\mu}$ pair.
    The $\mu^+$ can then go on to produce photons via bremsstrahlung or through annihilating with an ambient $\mu^-$ {\color{red}(there are no ambient muons?? -orel)}.
