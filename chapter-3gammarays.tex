...
\section{Production of TeV Gamma Rays}
  ...
  Instead, two WIMP dark matter paritlces may annihilate (directly or indirectly) into gamma rays, and this is discussed in Section~\ref{dm_spectral}.
  ...
  \begin{itemize}
    ...
    \item {\color{red}(format for explaining equations needs to be consistant, see equations \ref{eqn:inv_compt_avg_up_en}, \ref{eqn:pwn_pairprod} ??)}
  \end{itemize}
  ...
  % orel suggests looking at ICRC 2017 HESS electron spectrum up to 20 TeV?
  %   here: https://indico.snu.ac.kr/indico/event/15/session/5/contribution/694/material/slides/0.pdf
  %   its just a talk, can't find a proceeding of science for it
  ...
  With Equations~\ref{eqn:snr_prob} and \ref{eqn:snr_beta}, the spectrum from Equation~\ref{eqn:snr_spec} can be simplified with the following substitution:
  ...
  With Equation~\ref{eqn:snr_simplify}, equation~\ref{eqn:snr_spec} then simplifies to
  ...
  The bottom plot shows the differential spectra using the spectral indicies from the top plot.
  ...
  \begin{figure}[ht]
    ...
    \caption[Supernova Diffuse Acceleration Spectral Indicies]{
      ...
      {\color{red}(power laws produce the flux?? -parents)}
  ...
  The $\pi^{0}$ then quickly (\SI{8.5e-17}{s}~\cite{pdg2016} {\color{red}(seconds?? -parents)}) decays into two gamma rays.
  ...
  %Active galactic nuclei possess particle jets, which can accelerate particles when the jet shocks into the extragalactic medium.
  ...
  \subsection{Dark Matter Interactions}\label{dmgammaproduction}
    ...
    Another type are cryogenic {\color{red}detectors (such as?? -parents)}, which use pucks of germanium and silicon to measure WIMP collisions.
    ...
    A third example are scintillation detectors, which are built with {\color{red}crystals like titanium-doped sodium iodine (underlines?? -parents)}.
...
\section{Galactic Center}
  ...
  At its heart, kinematic observations of nearby stars have been used to infer the presence of a supermassive black hole, with a mass of \SI{4e6}{ \Msol{} }~\cite{sgra_massdist}.
  Around this black hole, gamma ray emission is observed~\cite{gc_pointsrc_hess,gc_pointsource_hess2,gc_veritas_pointsource,gc_magic_pointsource}, though the {\color{red}production mechanism (of what?? -parents)} is still under debate.
  There are other gamma-ray sources as well, including dust along the galactic plane and supernova remnants is there a graphic that illustrates the diversity.
  ...
  The debate between these two models is currently ongoing~\cite{gc_pev_or_pwn} {\color{red}(rewrite?? -parents)}.
  ...
  This occurs due to the upscattering of optical, infrared, and cosmic micrwave background photons.
...
\section{Indirect Dark Matter Search}
  ...
  \subsection{Dark Matter and Gamma Rays}
    ...
    The final spectrum of gamma rays used in this analysis is calculated in Section~\ref{dm_spectral} {\color{red}(spectra/spectrum?? -parents)}.
  ...
  \subsection{Spectrum of Gamma Rays from WIMP Annihilations}\label{dm_spectral}
    ...
    As WIMP properties are not well understood, all particle-antiparticle pairs can be considered as possible annihilation channels.
    In addition, WIMP annihilations may have branching ratios, where a population of WIMPs will annihilate into different channels with different probabilities.
    As a made up example, for 100 WIMP annihilations, 80 may annihilate into $b\bar{b}$, while the remaining 20 annihilate into a gluon-gluon $gg$ pair.
    However, branching ratios are beyond the scope of this analysis, so only the $100\% \rightarrow b\bar{b}$ annihilation channel is considered.
    ...
    {\color{red}(how does feet with thr?? -parents)}
    It is interesting to note that, if the entire VERITAS energy range of \SIrange{1.5}{70}{TeV} is used, the integrated photon spectrum increases the observed photon flux by a factor of 26.
  ...
  \subsection{Dark Matter Halo Structure}\label{dm_spatial}
    ...
    However, observations of dwarf galaxies instead have a flat density core within a given radius~\cite{flores1994observational,CoreVsCusp}.
    ...
    {\color{red}(tense is off in the two previous sentences?? -parents)}
    ...
    {\color{red}(what does this mean?? -parents)}
...
\section{Cherenkov Photons}\label{sec:cherenkov}
  ...
  Cherenkov light and its production is discussed in this next section, because the detection of gamma rays in Section~\ref{sec:atmoshowers} relies heavily on knowledge of Cherenkov light.
  ...
  {\color{red}(paragraph needs reworking, comments cut off?? -parents)}
...
\section{Atmospheric Showers}\label{sec:atmoshowers}
  ...
  {\color{red}(missing comment?? -parents)}
  ...
  In the initial collision, the astrophysical proton $p_{\textrm{cosmic}}$ interacts with an atmospheric nulceon $N$.
  ...
  These other particles include additional protons, neutrons, and other hyperons, as well as kaons, other mesons, and additional baryons and nucleons.
