...
\section{Indirect Dark Matter Search}
  ...
  \subsection{Spectrum of Gamma Rays from Dark Matter}\label{dm_spectral}
    ...
    These spectra are simulated using monte carlo simulations with PYTHIA~\cite{pythia} and HERWIG~\cite{herwig}.
    ...
    {\color{red}(Are these detailed simulations of the production and then decay of bb?? -orel )}
...
\section{Cherenkov Photons}\label{sec:cherenkov}
  ...
  As the section after this relies heavily on knowledge of Cherenkov light, Cherenkov light production is discussed here.
  Within the Earth's atmosphere (or any dielectric medium), any charged particles travelling at velocity $v > c_{atmosphere}$, where $c_{atmosphere}$ is the phase velocity of light in the atmosphere, will induce the atmosphere to produce Cherenkov photons~\cite{cherenkov}.
...
\section{Atmospheric Showers}
  ...
  These protons produce hadronic showers, which produce Cherenkov light.
  In order to accurately measure the gamma rays from a source, these hadronic showers must first be removed.
  ...
  {\color{red}?? section rewrite begins here ??}
