...
\section{Indirect Dark Matter Search}
  ...
  \subsection{Spectrum of Gamma Rays from Dark Matter}\label{dm_spectral}
    ...
    {\color{red}(how many gamma rays are produced by the halo (Not sure if I would point to equation 6.21. I would show and explain the equation here as this chapter describes the physics needed to understand the thesis. ?? -maria,orel ))}
    
    For example, with a $<\sigma{}v> = 3 \times 10^{-26} \f $
    
    $ \left < \sigma v \right > = 3 \times 10^{-26} \;\, \frac{\textrm{cm}^3}{\textrm{s}} $

    $ m_{\chi} = 10 \; \textrm{TeV} $

    $ 4 - 70 \textrm{TeV} $

    3 degree radius field-of-view
  
    NFW Halo, which has a J-Factor of ??
    
    Which results in flux X.  With 
    
    50,000 $m^2$ effective area

    100 hours observing time
    
    This results in Y photons in the telescope.
