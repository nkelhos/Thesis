...
\section{Methods of Creation}
  ...
  In hadronic processes, protons can be accelerated ($p_{accel}$) by fermi acceleraton or as part of an Active Galactic Nucleus jet~\cite{hadronic1,hadronic2}.
  Then, upon striking an ambient proton ($p_{ambient}$), the interaction will produce $\pi^{+}$, $\pi^{-}$, and $\pi^{0}$, as in Equation~\ref{eqn:protonproton}~\cite{pp_pion,pp_pion2}.
  
  \begin{equation}\label{eqn:protonproton}
    p_{accel} + p_{ambient} \rightarrow \pi^+ + \pi^+ + \pi^0
  \end{equation}
  
  {\color{red}(should it be mentioned that other particles can be produced, if I mention it later in Equation~\ref{eqn:protoncollision} ??)}

  The $\pi^{0}$ then quickly (\SI{8.5e-17}{s}~\cite{pdg2016}) decomposes into a $\gamma\gamma$ pair, with \nicetilde$15\%$ of the original proton's energy.
...
\section{Galactic Center}
  ...
  This emission is not modeled in this analysis due to time constraints {\color{red}(??)}.
...
\section{Indirect Dark Matter Search}
  ...
  \subsection{Dark Matter and Gamma Rays}
    ...
    The final spectrum of gamma rays use in this analysis is calculated in Section~\ref{dm_spectral}.
...
\section{Atmospheric Showers}
  ...
  The proton-proton collision then produces a set of pions, as in Equation~\ref{eqn:protoncollision}, as well as other particles (`...') that vary with the initial kinetic energy.
