...
\section{Methods of Creation}
  ...
  Then, upon striking an ambient atom, the proton will convert into $\pi^{+}$, $\pi^{-}$, and $\pi^{0}$~\cite{pp_pion,pp_pion2}.
...
\section{Indirect Dark Matter Search}
  ...
  \subsection{Dark Matter Halo Structure}\label{dm_spatial}
    ...
    where $\alpha$ is fixed to 0.17 from Ref.~\cite{PieriGalaxySims}.
    $r_s$ is {\color{red}(and was chosen how??)}.
    % for r_s, see gMW_TOT_RSCALE in clumpy halo parameter file
    % for \rho_s, see ??
    $\rho_s$ is {\color{red}(and was chosen how??)}.
    These parameters are taken from the best-fit values for a Milky-Way-like galaxy in Ref.~\cite{pieriGalaxySims}
