...
This discussion revolves around three main topics: astrophysical mechanisms that create gamma rays, the production of gamma rays from the dark matter halo around the Galactic Center, and gamma-ray-induced air showers in the Earth's atmosphere.

\section{Methods of Creation}

  There are several mechanisms that can produce photons with TeV energies.
  A gamma ray can start as a thermal photon, then gain the majority of its energy from interactions with electrons, referred to as a leptonic production.
  ...
  The primary mechanism this analysis searches for however, is when two WIMP dark matter particles annihilate either directly or indirectly into gamma rays, and is discussed in Section \ref{dmgammaproduction}.
  ...
  \subsection{Dark Matter Interactions}\label{dmgammaproduction}
    ...
    Often experiments make use of several of these signals, {\color{red}as well as the time difference between each observable (?? what do you mean? -Orel)}.
    However, to date no substantial dark matter signal has been detected with these methods~\cite{direct_dm_detection}.

    With Indirect searches, $\chi\chi \rightarrow ss$, astrophysical observations are analyzed for excess standard model particles, which are unexplained by known astrophysical processes.
...
\section{Galactic Center}
  ...
  The immediate area surrounding the Galactic Center contains a point-source gamma-ray emitter, though this mechanism is uncertain.
  ...
  This analysis instead focuses on the gamma-ray flux outside this point-like inner angular region.{\color{red}(not clean enough??)}
...
\section{Indirect Dark Matter Search}
  ...
  {\color{red}This (stronger word needed??)} is done indirectly, by searching for gamma-rays that are emitted when two dark matter particles annihilate.
  Since the rate of annihilation depends on the local dark matter density, the radially-dependent structure of dark matter halos also affects the analysis.
  ...
  \subsection{Dark Matter Halo Structure}\label{dm_spatial}
    Observations allow most galactic dark matter halos to be modeled by several similar density profiles.
    ...
    The amount of gamma rays produced by these annihilations can be calculated with Equation \ref{eqn:dmflux}.
    ...
    Velocity-averaging is used because the cross-section is velocity dependent, since the WIMPs that pass through a volume of space are likely to have a distribution of velocities {\color{red}(cite??)}.
  ...
  \subsection{Spectrum of Gamma Rays from Dark Matter}\label{dm_spectral}
    ...
    Each WIMP annihilation may directly produce different child particles, called an annihilation channel.
    WIMPs may annihilate directly into two gamma rays (the \gamma\gamma channel), or may instead produce a quark-antiquark pair (b\bar{b},t\bar{t}), a lepton-antilepton pair, or almost any other particle-antiparticle pair.
    ...
    The software package CLUMPY \cite{CLUMPYcode} is used to calculate the different produced gamma-ray spectra for each annihilation channel.
...
\section{Atmospheric Showers}
  ...
  The $\pi^{+}$ and $\pi^{-}$ have longer decay times ($\pi^{\pm} \rightarrow $\SI{3e-8}{s} vs $\pi^{0} \rightarrow $\SI{9e-17}{s} \cite{pdg_2014} ), allowing them to carry energy further away from the central shower axis.
  Both of these effects create sub-showers further away from the primary particle axis, which tends to cause hadronic showers (and their resulting Cherenkov images, discussed in Section \ref{sec:cherenkov}) to be wider than a purely electromagnetic shower of the same length. 
  ...
  The initial energies were chosen as a \SI{1}{TeV} proton shower will on average divest $\frac{1}{3}$ of its energy to the $\pi^{0}$.
  This $\pi^{0}$ then decays into an electromagnetic sub-shower depicted in the image, so these two showers had roughly the same initial available energy.
  ...
  However in an atmospheric shower, the number and distribution of charged particles and their velocities, as well as energy losses, tend to diffuse the theoretically-clean Cherenkov cones into a fuzzy pool of light on the ground.
