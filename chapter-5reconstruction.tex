...
The Crab Nebula acts as a known \textit{standard candle} source to test out the analysis, before attempting to search for dark matter within the Sgr A* data.
  ...
  \subsection{Energy Dispersion}\label{subsec:edisp}
    ...
    {\color{red}(add a sentence mentioning the interpolation ctools does in order to make it continuous to get Figure~\ref{fig:migmatrix} ??)}
...
{\color{red}(cant read comment about figure?? -parents)}
...
{\color{red}(previous sentence has ?? -parents)}
