...
\begin{table}[H]
  ...
  \begin{tabular}{lp{0.7\linewidth}}
    \textbf{width:}  & image angular width ; \\
    \textbf{length:} & image angular length ; \\
    \textbf{wol:}    & $\frac{\textrm{width}}{\textrm{length}}$ ; \\
    \textbf{size:}   & total image dc; \\
    \textbf{ntubes:} & number of pixels in the image; \\
    \textbf{loss:}   & fraction of image pixels at the edge of the camera; \\
    \textbf{asym:}   & distance between image center-of-dc and the pixel with the highest dc; \\
    \textbf{tgrad:}  & the slope of the linear time fit to the pixel arrival times; \\
    \textbf{cross:}  & angular distance between the image center and the average intersection point of the image axes. \\
  \end{tabular}
\end{table}
{\color{red}(fix this table??)}
