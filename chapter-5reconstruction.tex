...
\section{Position Reconstruction}\label{subsec:posrecon}
  ...
  % Once trained on these parameters, the machine learning algorithm can construct a boosted decision tree for determining any new image's \textit{disp}, which can then be used with the image axes intersections to improve the gamma-ray reconstructed positions.
    ...
    To examine the effect this has on an analysis, data events from the Crab Nebula and the Galactic Center are reconstructed with this disp method.
    The events are then histogrammed by their distance from their source, which are shown in Figure~\ref{fig:disp_event_offset}..
    With the Disp (green) events, the 68\% containment radius actually increases compared to the Geometic events.
    However, this may be due to the Disp method successfully reconstructing a few more events at the edge of the camera, which can be seen in the $\frac{\textrm{Disp}}{\textrm{Geom}}$ residual plots in Figure~\ref{fig:disp_event_offset}.
    Within \ang{2} of the camera center, the disp method appears to have no effect.
    The lack of improvement may be due to the use of boosted decision trees, whereas a previous study with this method utilized a lookup table of six parameters\cite{Beilicke2012NIM}.
    
    {\color{red}Again, you need to be more specific. you need to discuss the actual resolution obtained with both methods and state the improvement achieved with DISP. I think the RMS might be a better test of this and anyway the mean would be too affected by the tail. In fact I think the quantiles are better?? -Orel}
