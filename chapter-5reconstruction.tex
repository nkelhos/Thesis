...
with 30\% of the window before each pixel's $T_{0}$, to more accurately measure the charge due to Cherenkov photons in the pixel.
  ...
  \begin{figure}[b]
    \centering
    \includegraphics[width=0.65\textwidth]{images/shower_parameter_diagram/diagram.pdf}
...
\section{Position Reconstruction}\label{subsec:posrecon}
  ...
  These axes should intersect {\color{red}very close together (why not exactly?? -maria)}, and the average of the intersection points determines the event's initial direction.
  ...
  However, {\color{red}no significant benefit was achieved (why mention it here?? now there isn't much motivation to read further)}.
  ...
  % Once trained on these parameters, the machine learning algorithm can construct a boosted decision tree for determining any new image's \textit{disp}, which can then be used with the image axes intersections to improve the gamma-ray reconstructed positions.
    ...
    Once the training is complete, it is tested on a separate set of {\color{red}17,000 (why this many?? -maria)}simulated events, which have their true and predicted disps plotted in Figure~\ref{fig:disptraining}.
    ...
    To examine the effect this has on an analysis, the point spread functions (PSFs) can be compared.
  ...
  \subsection{Background Models}\label{background_production}
    ...
    \begin{figure}[!p]
      ...
      \caption[CTOOLS Background Fine Energy Bins]{
        The V5 {\color{red}Background's (?? -maria)} fine energy bins.
...
\section{Camera Studies}
  ...
  \subsection{Background Structure at the Low Energy Threshold}\label{subsec:bkgstructure}
    ...
    As the shape of the atmospheric interaction volume strongly depends on the telescope elevation, a small elevation range helps isolate atmosphere-dependent effects {\color{red}(is this better?? -maria)}.
  ...
  \subsection{Effects of Disabled Pixels}
    ...
    {\color{red}While this single-pixel loss-of-events effect was notable, it was not incorporated into the main analysis of this thesis (Then I would write something like "The effect is very small and thus negligible, and move it to the end, as it somehow demotivates to read further as it is not needed for your analysis.)}.
