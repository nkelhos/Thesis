...
The Crab Nebula acts as a known \textit{standard candle} source to test out the analysis, before attempting to search for dark matter within the Sgr A* data.
  ...
  \subsection{Background Models}\label{background_production}
    ...
    Each template is made by binning all background events radially by camera position and a separate second time by their energy.
    Then an interpolating function is applied to each to produce smooth spatial and spectral components of the template.
