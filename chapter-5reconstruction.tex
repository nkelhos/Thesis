...
%In this equation, $l_{\textrm{i}}$ and $w_{\textrm{image}}$ are each observed image's length and width.
In this equation, $l_{i}$ and $w_{i}$ are each observed image's length and width.
Multiple images of the same shower can be combined to calculate the $\textrm{MRSL}_{\textrm{shower}}$ and $\textrm{MRSW}_{\textrm{shower}}$ for each shower.
...
Gamma-ray showers and proton showers have different distributions of MRSL and MRSW (proton showers tend to be wider at a given length), so cuts are applied based on these two parameters to partially filter out the proton showers.
In this analysis, only events that have a $\textrm{MRSL} < 0.7$ and $\textrm{MRSW} < 0.5$ are used.
...
The Crab Nebula acts as a known \textit{standard candle} source to test out the analysis, before attempting to search for dark matter within the Sgr A* data.
  ...
  \subsection{Background Models}\label{background_production}
    ...
    Once the templates are built, each chunk of data (a fraction of an observation run) is given an independent copy of its epoch-specific background template.
    The templates are constructed from dark observations, detailed later in Section~\ref{veritasdata}.
    %Each template is made by binning all background events radially by camera position and then a separate second time by their energy.
    Each template is made by combining two independent binnings of the background events.
    The events are first binned radially to model the spatial behavior, then separately the same events are binned in energy to model the spectral behavior.
    Then an interpolating function is applied to each binning to produce smooth spatial and spectral components of the template.
    The spectral bins and interpolated function can be seen in Figure~\ref{fig:background_profile}.
    The radial bins and interpolated function at two different energies is shown in Figure~\ref{fig:background_radial}.
    ...
    The function $f(e,x,y)$ has units of $\frac{\textrm{Number of Counts}}{ \textrm{MeV} \times \textrm{s} \times \textrm{sr} }$.
    %The function $M_{e}(e)$ is a spectral function power law, primarily used to account for variations in the measured energy spectrum of the background models at different elevations.
    The function $M_{e}(e)$ is an energy-dependent power law correction function, primarily used to account for variations in the acceptance of the background models at different elevations.
...
Figure~\ref{fig:migmatrix} shows one migration matrix, representative of all migration matrices used in this analysis.
For all matrices in this analysis, the bias between \SIrange{4}{70}{TeV} is similar.
...
%      {\color{red}(split into two figures??)}
%    }
...
\begin{figure}[p]
  ...
  \begin{tabular}{rl}
    %\newcommand{\lwidth}{6.5}
    %\newcommand{\rwidth}{4}
    \subfloat{\includegraphics[width=8.5cm]{images/background_gradient_replot/sgraoff_1_hist_energy.pdf}} & 
    \subfloat{\includegraphics[width=5.0cm]{images/background_gradient_replot/sgraoff_1_cam.pdf}}         \\
    \subfloat{\includegraphics[width=8.5cm]{images/background_gradient_replot/sgraoff_2_hist_energy.pdf}} & 
    \subfloat{\includegraphics[width=5.0cm]{images/background_gradient_replot/sgraoff_2_cam.pdf}}         \\
    \subfloat{\includegraphics[width=8.5cm]{images/background_gradient_replot/sgraoff_3_hist_energy.pdf}} & 
    \subfloat{\includegraphics[width=5.0cm]{images/background_gradient_replot/sgraoff_3_cam.pdf}}         \\
  \end{tabular}
  \caption[Atmospheric Gradient at Low Energies in the VERITAS Camera]{
    ...
    The orange dot and circle denote the average event position, and 68\% containment radius for the events in the plot.
    The \SIrange{1.8}{3.2}{TeV} energy range shows events are detected more in the upper half of the camera.
    From \SIrange{4}{6}{TeV}, the mean even position has shifted to the bottom half of the camera.
  }
...
\begin{figure}[p]
  \centering
  \begin{tabular}{rl}
    \subfloat{\includegraphics[width=8.5cm]{images/background_gradient_replot/sgraoff_5_hist_energy.pdf}} & 
    \subfloat{\includegraphics[width=5.0cm]{images/background_gradient_replot/sgraoff_5_cam.pdf}}         \\
    \subfloat{\includegraphics[width=8.5cm]{images/background_gradient_replot/sgraoff_6_hist_energy.pdf}} & 
    \subfloat{\includegraphics[width=5.0cm]{images/background_gradient_replot/sgraoff_6_cam.pdf}}         \\
    \subfloat{\includegraphics[width=8.5cm]{images/background_gradient_replot/sgraoff_7_hist_energy.pdf}} & 
    \subfloat{\includegraphics[width=5.0cm]{images/background_gradient_replot/sgraoff_7_cam.pdf}} 
  \end{tabular}
