...
A group of these median widths and spreads-in-width are shown in Figure~\ref{fig:mscw_params}.
...
\begin{figure}[bh]
  ...
  \caption[Shower Width Median and Spread]{
...
These stored lengths and widths can then be used to create new variables \textit{mean scaled length} (MSCL) and \textit{mean scaled width} (MSCW), calculated with Equation~\ref{eqn:mscwl}.
In this equation, $l_{\textrm{shower}}$ and $w_{\textrm{shower}}$ are each observed shower's length and width.
...
Gamma-ray showers and proton showers have different distributions of MSCW and MSCL (proton showers tend to be wider at a given length), so cuts can be applied based these two parameters to partially filter out the proton showers.
...
{\color{red}(add gamma vs proton mscw histograms??)}
