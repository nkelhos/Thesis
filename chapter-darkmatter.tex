...
\section{Astrophysical Evidence for Dark Matter}
  ...
  \subsection{$10^{19}\:\text{m}$ : Dwarf Galaxy Scale}\label{dm_dwarfscale}
    ...
    \begin{equation}\label{eqn:virial}
      2 \text{KE} = \text{PE}
    \end{equation}
    Thus, by measuring stellar velocities and calculating the total kinetic energy of a group of stars, the potential energy can be calculated, which indicates the amount of mass is present in the group.
    ...
    From the Maxwell-Boltzmann distribution, the velocity probability density function is Equation \ref{eqn:mb_vel3d}:
    ...
    In one dimension, each velocity distribution is Equation \ref{eqn:mb_vel1d}, a normal distribution.
    \begin{equation}\label{eqn:mb_vel1d}
      f_v(v_i) = \sqrt{ \frac{m}{2\pi k T} } \text{exp} \left [ - \frac{m v_i^2}{2kT} \right ]
    \end{equation}
    with the mean velocity in each direction $\mu_{v_i} = 0$ and a standard distribution is $\sigma_{v_i} = \sqrt{\frac{kT}{m}}$
    ...
    \end{equation}
    And, with a Maxwell distribution, the most probable speed $\left \langle v \right \rangle = \frac{2}{\sqrt{\pi}} \sqrt{\frac{2kT}{m}}$, thus measuring the mean speed and distribution of speeds of a group of stars can yield the total mass of the system in Equation \ref{eqn:mb_total}:
    {\color{red}(THIS MAXWELL STUFF IS WRONG, THE MASS $m$ CANCELS, NEEDS TO BE REDONE!??)}
    
    Thus, by measuring the distribution of star velocities and the average temperature of the system {\color{red}(what is temp T of the system??  does it come from $v_p$??  clean up this paragraph??)}, the total mass of system can be calculated: $$m = \frac{3kT}{\sigma^2}$$.
    As stellar velocity measurements rely on doppler-shifted spectral lines, and not the star's absolute brightness, this mass estimate is fairly resistant to changes in observed brightness, either due to atmospheric variations during telescope observations, or to the amount of light-absorbing dust in the line-of-sight.
  ...
  \subsection{$10^{20}\:\text{m}$ : Galaxy Scale}
    ...
    Then, like in Section \ref{dm_dwarfscale}, the amount of light produced can be compared with the Sun as a standard mass-to-light ratio, allowing for a prediction of the amount of mass contained in that sector.
...
\section{Particle Dark Matter}
  ...
  However, it was eventually demonstrated that because of these relativistic speeds, neutrinos would be able to smear out any of their initial overdensities.
  ...
  When observations are made of earlier times, its found to be the opposite: galaxy-scale gravitational wells form first, then cluster-scale wells, then super-cluster-scale wells in present times.
  As this is the opposite of whats expected for relativistic dark matter, neutrinos were ruled out as a dark matter candidate {\color{red}(cite??)}.
